...
def run_fresh_comparison():
    ...
    try:
        ...
        print("✅ STEP 1 SUCCESS: Scraping and matching completed")
...
def find_latest_comparison_file():
    ...
    try:
        ...
        if not files:
            print("❌ No keyboard_comparison_*.xlsx files found")
            return None
        ...
        print(f"✅ STEP 2 SUCCESS: Found latest file: {latest_file}")
...
def upload_comparison_file(file_path):
    ...
    try:
        ...
        # Check result
        if result.returncode != 0:
            print(f"❌ STEP 3 FAILED: Exit code {result.returncode}")
            print(f"Error output: {result.stderr}")
            return False
        
        print("✅ STEP 3 SUCCESS: Upload completed")
        print(f"Output: {result.stdout}")
        return True
...
def main():
    ...
    if not success:
        print("\n❌ ORCHESTRATION FAILED: Step 1 (Scrape & Match) failed")
    ...
    if latest_file is None:
        print("\n❌ ORCHESTRATION FAILED: Step 2 (Find file) failed")
    ...
    if not success:
        print("\n❌ ORCHESTRATION FAILED: Step 3 (Upload) failed")
        sys.exit(1)
